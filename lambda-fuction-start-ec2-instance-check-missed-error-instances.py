...
def lambda_handler(event, context):
    ...
    for reservation in instances['Reservations']:
        for instance in reservation['Instances']:
            ...
            print(f"Checking instance: {instance_name} ({instance_id}) - State: {instance_state}, Start Time: {start_time}, Snoozing Tag: {snoozing_status}")
            ...
            # Start the instance if conditions match
    ...
    if affected_instances or missing_tag_instances:
        sns.publish(TopicArn=SNS_TOPIC_ARN, Message=email_message, Subject="AWS EC2 Start Notification")
